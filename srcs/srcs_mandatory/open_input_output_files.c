...
/*   Updated: 2023/03/11 15:53:34 by vcedraz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_input_output_files(t_data *d)
{
	...
	if (-1 == access(d->args.file1, R_OK | F_OK))
		handle_error(d, "input file access", &free_error2, 2);
	fds->input_fd = open(d->args.file1, O_RDONLY);
	if (-1 == fds->input_fd)
		handle_error(d, "open input file", &free_error2, 2);
	fds->output_fd = open(d->args.file2, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (-1 == fds->output_fd)
		handle_error(d, "open output file", &free_error2, 2);
}
